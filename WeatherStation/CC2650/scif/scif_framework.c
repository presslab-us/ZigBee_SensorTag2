...
#elif defined(__TI_COMPILER_VERSION__) || defined(rvmdk) || defined(__ARMCC_VERSION)
    return __clz(__rbit(x));
#else
...
SCIF_RESULT_T scifInit(const SCIF_DATA_T* pScifDriverSetup) {
    ...
    HWREG(AUX_EVCTL_BASE + AUX_EVCTL_O_VECCFG1) = AUX_EVCTL_VECCFG1_VEC2_EV_AON_SW      | AUX_EVCTL_VECCFG1_VEC2_EN_M |
...
/** \brief Returns a bit-vector indicating the ALERT events associcated with the last ALERT interrupt
  ...
  *     - [23:16] Normal task completion, without error, one bit per task ID
...
uint32_t scifGetAlertEvents(void) {
    uint32_t bvTaskIoAlert = scifData.pTaskCtrl->bvTaskIoAlert;
    return (scifData.pTaskCtrl->bvTaskDoneAlert << 16) | bvTaskIoAlert;
} // scifGetAlertEvents
...
void scifAckAlertEvents(void) {
    ...
    HWREG(AUX_EVCTL_BASE + AUX_EVCTL_O_VECCFG1) |= AUX_EVCTL_VECCFG1_VEC2_POL_M;
    HWREG(AUX_EVCTL_BASE + AUX_EVCTL_O_VECCFG1) &= ~AUX_EVCTL_VECCFG1_VEC2_POL_M;
